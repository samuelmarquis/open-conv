...
pub(super) struct OpenConvAudioPorts {
    layout: Arc<AudioLayoutStore>,
}

impl OpenConvAudioPorts {
...
impl PluginAudioPortsExtension for OpenConvAudioPorts {
...
/// `&mut self` lock (see [`OpenConvPlugin`](super::OpenConvPlugin)). This does not mean
/// changes are allowed while active — the adapter enforces that this is only called when
/// no `Processor` exists (inactive).
pub(super) struct OpenConvConfigurableAudioPorts {
    layout: Arc<AudioLayoutStore>,
}

impl OpenConvConfigurableAudioPorts {
    pub(super) fn new(layout: Arc<AudioLayoutStore>) -> Self {
        Self { layout }
    }
}
...
impl PluginConfigurableAudioPortsExtension for OpenConvConfigurableAudioPorts {
